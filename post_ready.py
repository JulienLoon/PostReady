...
class PostReadyForm(npyscreen.FormBaseNew):
    def create(self):
        # --- Header bovenaan het scherm ---
        self.header = self.add(npyscreen.FixedText, value="=== POSTREADY CONFIGURATOR ===", editable=False, color='STANDOUT', rely=1, relx=2)
        self.add(npyscreen.FixedText, value="Use arrow keys and space to select options", editable=False, rely=3, relx=2, color='CURSOR_INVERSE')

        row = 5

        # --- Pre-template Cleanup ---
        self.add(npyscreen.FixedText, value="Pre-template Cleanup", editable=False, rely=row, relx=2, color='LABEL')
        row += 1
        ...
        # --- Initial Setup ---
        self.add(npyscreen.FixedText, value="Initial Setup", editable=False, rely=row, relx=2, color='LABEL')
        row += 1
        self.use_dhcp = self.add(npyscreen.Checkbox, name="Use DHCP", rely=row, relx=4)
        row += 1
        self.static_ip = self.add(npyscreen.TitleText, name="Static IP:", rely=row, relx=4, begin_entry_at=15)
        row += 1
        self.gateway = self.add(npyscreen.TitleText, name="Gateway:", rely=row, relx=4, begin_entry_at=15)
        row += 1
        self.dns = self.add(npyscreen.TitleText, name="DNS:", rely=row, relx=4, begin_entry_at=15)
        row += 2

        # --- Loose Components ---
        self.add(npyscreen.FixedText, value="Loose Components", editable=False, rely=row, relx=2, color='LABEL')
        row += 1
        ...
        row += 1
        self.create_user = self.add(npyscreen.TitleText, name="Create user:", rely=row, relx=4, begin_entry_at=15)
        row += 2

        # --- Start button onderaan ---
        self.start = self.add(npyscreen.ButtonPress, name="START", rely=row, relx=2, when_pressed_function=self.on_start)
